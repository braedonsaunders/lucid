#!/usr/bin/env python3
"""Compare capture routes inside one owned installed-extension session."""
import argparse
import gzip
import hashlib
import json
...
def main():
    ...
    ap.add_argument('--capture-route-order',nargs='+',choices=['worker','surface'],required=True,
                    help='Benchmark-only phases in one installed session; requires the switchable prototype')
    ...
    if args.capture_route_order and (not args.presentation_trace or args.samples % (30*len(args.capture_route_order))):
        ap.error('route phases require tracing and a multiple of 30 samples per phase')
    ...
    report['capture_route_order']=args.capture_route_order
    ...
    try:
        ...
        for index,label in enumerate(report['order']):
            ...
            for start in range(0,args.samples,30):
                route=None;phase=None
                if args.capture_route_order:
                    per_phase=args.samples//len(args.capture_route_order)
                    phase=start//per_phase;route=args.capture_route_order[phase]
                    if start % per_phase == 0:
                        js('''async page => {
                          await page.evaluate(route=>document.documentElement.dataset.lucidCaptureMode=route, ROUTE);
                          await page.waitForTimeout(5000);
                          return {purpose:'capture-route',route:ROUTE};
                        }'''.replace('ROUTE',json.dumps(route)))
                chunk=js('''async page => {
              ...
              return result;
                ...
                if route:
                    expected='surface-transfer' if route=='surface' else 'port '
                    if not all(expected in row['frameStats'] for row in chunk['samples']):
                        raise RuntimeError('requested capture route did not remain active')
                    chunk['capture_route']=route;chunk['capture_phase']=phase
            ...
            chunk_path=args.out/f'{index}-{label}-chunks.json'
            samples['phase_chunks']={'file':chunk_path.name,'sha256':digest(chunk_path)}
            if args.presentation_trace:
                trace=js('''async page => {
                  const surfaces=[];
        ...
        validated=analyze(args.out/'report.json',minimum_seconds=args.samples)
        phase_report={'purpose':'within-session capture route comparison; not a sustained release gate',
                      'validated_trace':validated,'runs':[]}
        for run in report['runs']:
            measurement=run['measurement']
            path=args.out/measurement['phase_chunks']['file']
            if digest(path)!=measurement['phase_chunks']['sha256']:raise ValueError('phase chunks changed')
            phase_chunks=json.loads(path.read_text())
            trace=json.loads(gzip.decompress((args.out/measurement['presentation_trace']['file']).read_bytes()))
            records=trace['surfaces'][0]['samples']
            phases=[];pooled={}
            for phase,route in enumerate(args.capture_route_order):
                selected=[chunk for chunk in phase_chunks if chunk['capture_phase']==phase]
                if not selected or any(chunk['capture_route']!=route for chunk in selected):raise ValueError('phase route mismatch')
                start,end=selected[0]['startedEpoch'],selected[-1]['endedEpoch']
                latency=[row['latencyMilliseconds'] for row in records if start<=row['at']<end]
                duration=(end-start)/1000
                if not latency or duration<args.samples/len(args.capture_route_order):raise ValueError('incomplete phase')
                phases.append({'phase':phase,'route':route,'start':start,'end':end,'seconds':duration,
                               'actual_draws':len(latency),'actual_fps':len(latency)/duration,'p95_ms':percentile(latency,.95)})
                group=pooled.setdefault(route,{'seconds':0,'latencies':[]})
                group['seconds']+=duration;group['latencies'].extend(latency)
            phase_report['runs'].append({'phases':phases,'pooled':{route:{'seconds':group['seconds'],
                'actual_fps':len(group['latencies'])/group['seconds'],'p95_ms':percentile(group['latencies'],.95)}
                for route,group in pooled.items()}})
        (args.out/'phase-comparison.json').write_text(json.dumps(phase_report,indent=2)+'\n')
    finally:
        if session:
            try:
