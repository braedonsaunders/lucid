...
def main():
    ...
    if args.frames < 12 or args.width % 16 or height % 2:
    ...
    for source_id, path in args.source:
        ...
        if int(info['width']) < args.width * 4 or int(info['height']) < height * 4:
        ...
        run(['-i', str(path), '-frames:v', str(args.frames), '-an', '-vf',
             f'scale={args.width*4}:{height*4}:flags=lanczos:in_color_matrix=bt709:out_color_matrix=bt709:in_range=tv:out_range=tv',
        ...
        for codec in ('h264', 'vp9'):
            for bitrate in (120_000, 400_000):
                ...
                records.append({'id': identity, 'source_id': source_id,
                    ...
                    'width': args.width, 'height': height, 'scale': 4,
