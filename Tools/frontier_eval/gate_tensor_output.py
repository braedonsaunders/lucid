#!/usr/bin/env python3
"""Check the fixed native full-4x tensor-output regression, without promotion."""
import argparse
import json
...
CORPORA = {
 'development48': ('aac663ad088fede87e1ede68cb46501603fc0f0b066b5ae6c07726b6555d923f', 'a42d1514b0c0ebb6b10d7107fe8c7d944022c9cca4a9f20992e560448ef4f696', 48, 4),
 'regression960': ('307d6670ef1c4918798172a16292dd55ade37db61d953efeddb5d60e21154b3b', '788123b0973b0a636dda6a21c1e2f8e9a5e2d58fc565f3dcbaad0a0853815076', 960, 10),
}
GATE = {
...
def evaluate(report, manifest, config, sequences):
    frames_hash, sequences_hash, count, stride = CORPORA[manifest['presentation_corpus']]
    ...
    if (manifest['source_manifest_sha256'] != sequences_hash
            or manifest['frozen_frames_manifest_sha256'] != frames_hash
            or manifest.get('output_storage') != 'tensor4x_fp32'
            or manifest.get('preserve_display_gain') is not False):
        raise ValueError('fixed corpus or nominal gain changed')
    if (config['candidate_sha256'] != SHIPPING or config['radius'] != 4
            or config.get('output_storage') != 'tensor4x_fp32'
            or config['tuning']['sharpness'] != .75):
        raise ValueError('configuration differs from frozen presentation')
    expected = {(s['source_id'], s['id'], frame) for s in sequences
                for frame in range(0, s['frames'], stride)}
    ...
    result.update(thresholds=GATE, promotion_authorized=False,
                  status='Previously evaluated native source regression; color coverage, browser cadence and fresh release validation remain')
    return result
...
def main():
    ...
    if digest(args.sequences) != CORPORA[manifest['presentation_corpus']][1]:
