...
$lock=[IO.File]::Open((Join-Path (Get-Location) 'fetch-grouped-owner.lock'),'CreateNew','ReadWrite','None')
...
try {
 ...
 Run-Python 'reds-fetch-grouped' @('-u','Tools/experiments/fetch_reds_subset.py','--index','reds-archive-index.json','--out','reds-frames','--sequences','60','--frames','100','--workers','4','--grouped-ranges')
