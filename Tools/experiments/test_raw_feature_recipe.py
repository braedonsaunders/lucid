...
import train_raw_feature_span as trainer
...
class RawFeatureRecipeTests(unittest.TestCase):
    def test_sequence_gradient_reaches_past_only_with_memory(self):
        torch.set_num_threads(2)
        torch.manual_seed(58)
        model = RawFeatureSPAN(Unshuffled(8, scale=2))
        with torch.no_grad():
        ...
        for use_history in (True, False):
            output = run_raw_feature_sequence(model, source, use_history=use_history)
    ...
    def test_four_arm_training_roundtrip_and_capacity_control(self):
        torch.set_num_threads(2)
        torch.manual_seed(58)
        ...
        for joint in (False, True):
            for current_only in (False, True):
                with self.subTest(joint=joint, current_only=current_only), tempfile.TemporaryDirectory() as tmp:
                    root = Path(tmp)
                    ...
                    with ExitStack() as stack:
                        ...
                        trainer.main()
                    loaded, checkpoint = load_raw_feature_checkpoint(out / 'step000002.pth')
                    ...
                    variants = {v: {r['frame']: r['metrics'] for r in report['rows'] if r['variant'] == v}
                                for v in ('base', 'initial', 'current_only', 'raw_features')}
                    self.assertEqual(variants['raw_features'][0], variants['current_only'][0])
                    if current_only:
                        self.assertEqual(variants['raw_features'], variants['current_only'])
                    if not joint:
