#!/usr/bin/env python3
"""Matched same-frame pre-TAA tap against a current-only control.

Arm A (--tap-mode none) is bit-for-bit the ordinary single-frame SR path: the
model sees only the preprocessed (debanded, TAA-accumulated) frame. Arm B
(--tap-mode full) additionally feeds the raw decoded current frame at the same
timestep through a zero-initialized side path. --tap-mode luma ablates a
single-channel luma-residual tap. No cross-frame alignment, no state, no
recurrence. All arms start at exactly the original SR function. After learning,
arm output need not equal the old backbone. Source-proxy diagnostics only;
native export, warp parity and timing unverified.
"""
import argparse
import copy
import json
...
from architectures.tap_span import TapSPAN, TAP_MODES
...
def validate(model, data, source_ids, *, source_motion_policy='search', baseline=None):
    model.eval()
    ...
    for lr, hr, identity in data:
        ...
        output, inputs = run_tap_sequence(model, source,
...
def main():
    ...
    ap.add_argument('--source-motion-policy', choices=('raw', 'taa', 'search'), default='search')
    ap.add_argument('--tap-mode', choices=TAP_MODES, default='full')
    ap.add_argument('--reds-bank', type=Path, help='Include fixed 16-frame held-out REDS validation')
    ap.add_argument('--joint', action='store_true', help='Optimize the fused SR backbone along with the tap branch')
    ...
    model = TapSPAN(base, train_backbone=args.joint, tap=args.tap_mode).cuda().train()
    ...
    if not groups[0]['params'] and not args.joint:
        ap.error('control arm requires --joint; a disabled tap has no branch parameters')
    if args.joint:
    ...
    files = ['Tools/architectures/tap_span.py',
             'Tools/experiments/tap_frame_feeding.py',
             'Tools/experiments/train_tap_span.py',
    ...
    experiment = {'args': {k: str(v) if isinstance(v, Path) else v for k, v in vars(args).items()},
        ...
        'recipe': 'final-frame HR reconstruction; FP32 fused SR; same-frame pre-TAA tap, no carried state',
        'backbone_trainable': args.joint,
        'optimizer': 'AdamW/cosine; tap branch 2e-4, joint backbone 2e-5; no weight decay',
        ...
        'motion': 'decoded-LR native integer search plus half-pixel refinement for evaluation only; training frames are same-timestep, no warping',
        'limitations': 'crop-local same-frame tap; no native warp/cost parity; no browser timing or release admission',
    ...
    for step in range(1, args.steps + 1):
        ...
        if not torch.isfinite(loss):
            raise ValueError('nonfinite tap objective')
        loss.backward()
