#!/usr/bin/env python3
"""Matched raw-observation feature memory versus current-only capacity control.
...
from architectures.raw_feature_span import RawFeatureSPAN
...
def validate(model, data, source_ids, *, use_history=True, baseline=None,
    ...
    for lr, hr, identity in data:
        ...
        output, inputs = run_raw_feature_sequence(model, source, use_history=use_history,
        ...
        current_only = run_raw_feature_sequence(model, source, use_history=False,
            source_motion_policy=source_motion_policy)
        variants = [('base', base), ('current_only', current_only), ('raw_features', output)]
    ...
    return {'complete': True, 'scope': 'source-disjoint full-bank-sequence patch diagnostic; not native playback',
            'use_history': use_history, 'source_motion_policy': source_motion_policy,
...
def main():
    ...
    model = RawFeatureSPAN(base, train_backbone=args.joint).cuda().train()
    ...
    files = ['Tools/architectures/raw_feature_span.py',
             'Tools/experiments/raw_feature_frame_feeding.py',
             'Tools/experiments/train_raw_feature_span.py',
    ...
    experiment = {'args': {k: str(v) if isinstance(v, Path) else v for k, v in vars(args).items()},
        ...
        'limitations': 'crop-local raw state; RGB warp computed but discarded; no native warp/cost parity; no browser timing or release admission',
    ...
    for step in range(1, args.steps + 1):
        ...
        output, inputs = run_raw_feature_sequence(model, source, use_history=not args.no_history,
    ...
    torch.save({'model': model.state_dict(), 'architecture': 'raw_feature_span2x', 'scale': 2,
                'state_representation': 'aligned_raw_features_v1',
                'feature_source': 'decoded_rgb8',
                'source_motion_policy': args.source_motion_policy,
