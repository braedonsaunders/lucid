...
Set-Location 'C:\lucid\quality-breakthrough-20260908-r33'
...
try {
 ...
 # Same proven stage ablation, now initialized from the actual shipping model.
 foreach($arm in @('big_control_1k','big_output_1k')){
  $arguments=$base+@('--out',$arm)
  if($arm -eq 'big_output_1k'){$arguments+=@('--native-output-stages')}
