...
The later stateful native graphs reduce explicit-history cost, but the causal students still trail shipping quality. Increasing width, hierarchical feature loss and pixel-target distillation did not establish a joint quality win. `shipping-direct2x.md` records the stronger route: preserve shipping reconstruction and fold its output head to the requested 2× size. On M4 Pro, native prediction plus identically sized NV12 packets falls from 45.7 to 20.0 ms at 720p→1440p, and 25.2 to 11.6 ms at 540p→1080p. These omit capture and browser delivery. The folded initialization established the performance baseline. The subsequent calibrated adversarial candidate now clears the development spatial gate; no new checkpoint is promoted before the frozen holdout and native quality checks.

PixRestore's tiled teacher improves perceptual quality but loses real detail relative to shipping. A fixed 50/50 teacher/shipping target preserves more detail while improving perceptual distances. Pixel regression fails to transfer that joint benefit. The completed RTX 4080 arm uses the **released PixRestore multi-layer DINO discriminator**, checked by source hash, against real training references. Raw adversarial output improves perception but loses too much fine fidelity. Fixed parameter calibration selected on development recovers the joint gate: 3.49% LPIPS and 7.13% DISTS improvement with all source detail limits met. The selected weights were frozen before the eight-source holdout. The initial added output-head gradient was 10.9% of reconstruction, but rose above it during training; it is not uniformly a small perturbation. The feature encoder/discriminator are training-only. Flicker work remains deferred at the owner's direction.
...
The subsequent source-PTS runs establish 58.38 distinct frames/s at 360p60 and 29.99 at 480p30 over more than ten minutes, on the local installed Chrome fixture. The separate 720p source-resolution quality screen fails and that resolution remains disabled. These measurements do not cover Safari, arbitrary third-party sites or physical display scanout.

[Mergeable subspace adaptation](subspace-adapter-protocol.md), informed by June 2026 FreqOrtho-SR, now has a completed matched control. Constraining actual weight updates preserves the selected layer subspaces but still fails image detail guards despite perceptual gains. No weights are promoted.

## September 3 research checked on September 5: SPARK

[SPARK](https://arxiv.org/html/2609.03813v1) adapts frozen diffusion-transformer SR models through input-conditioned affine modulation of selected activation channels. Its controller uses pooled VAE latents and modulates eight channels per stream/block. The paper reports concentrated activation energy and compares dominant-channel interventions with random/bottom-ranked controls. Training uses L40S GPUs and three existing DiT SR backbones; it does not establish Mac latency. No author implementation/checkpoint was located in the paper or targeted repository search; the unrelated SparkVSR project is a different method.

A compact convolutional backbone cannot inherit the paper's massive-activation premise by analogy. A useful next diagnostic would measure channel concentration and intervention sensitivity on training sources before deciding whether dynamic activation control is justified. That would differ from the just-rejected fixed weight-subspace constraint. No SPARK reproduction, gain or deployment is claimed.
