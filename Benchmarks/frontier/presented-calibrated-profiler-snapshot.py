    ...
    for size in args.sizes:
        ...
        for label, graph in models.items():
            ...
            converted.user_defined_metadata['lucid.checkpoint_sha256'] = digest(
                args.direct_checkpoint if label == direct_label and args.direct_checkpoint else args.checkpoint)
            converted.user_defined_metadata['lucid.output_scale'] = '4' if label == 'shipping4x' else '2'
            converted.user_defined_metadata['lucid.transformation'] = 'area phase folding' if label == 'direct2x_area' else 'trained weights'
