...
Set-Location 'C:\lucid\reference-detail-20260905'
if(Get-CimInstance Win32_Process | Where-Object {$_.Name -match '^python(w)?\.exe$'}) {throw 'Preserve existing Python jobs'}
foreach($path in @('calibrated','evaluation.json','evaluation-result.json','bank-validation.json','bank-validation-result.json','owner.lock')) {if(Test-Path $path){throw "Preserve existing $path"}}
$expected=@{
    'C:\lucid\presented-diversity-eval-20260905\area-init.pth'='8dd69c03424003aa98585edcbbc6c659d882a94fa680efba7d784a28deba241a';
...
$result=Get-Content 'reference-result.json' -Raw | ConvertFrom-Json
...
try {
    ...
    & 'C:\lucid\.venv\Scripts\python.exe' Tools/experiments/interpolate_presented_detail.py --anchor 'C:\lucid\presented-diversity-eval-20260905\area-init.pth' --candidate 'reference/step008000.pth' --weights 0.8 --out calibrated
    if($LASTEXITCODE -ne 0){throw 'Fixed interpolation failed'}
    $arguments=@('-u','Tools/frontier_eval/score_checkpoint_frames.py','--device','cuda','--frames','C:\lucid\presented-frozen-eval-20260904\frames','--checkpoint','reference80','calibrated/blend-0.8.pth','--checkpoint','reference_raw','reference/step008000.pth','--checkpoint','shipping','C:\lucid\presented-detail-20260904\shipping.pth','--present-4x-at-2x','shipping','--report','evaluation.json')
    ...
    $bankFrames='C:\lucid\anchored-detail-20260905\bank-validation-frames'
    if((Get-FileHash ($bankFrames+'\manifest.json')).Hash -ne 'eb8ad3792cc1999f4dc593494f959656be28b815ee21d3a9094c47b683f3a7fe'){throw 'Bank validation frames changed'}
    $arguments=$arguments | ForEach-Object {if($_ -eq 'C:\lucid\presented-frozen-eval-20260904\frames'){$bankFrames}elseif($_ -eq 'evaluation.json'){'bank-validation.json'}else{$_}}
    ...
    if($job.ExitCode -ne 0){throw 'Bank validation failed'}
