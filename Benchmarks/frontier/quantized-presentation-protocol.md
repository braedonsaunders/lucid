...
## Frozen wider regression

Keep the exact native graphs, Standard settings and nominal-gain correction for all eight previously evaluated source families, both codecs, both bitrates, 300 consecutive frames per condition and spatial stride ten: 960 paired outputs. Use the existing frame manifest SHA `307d6670ef1c4918798172a16292dd55ade37db61d953efeddb5d60e21154b3b`. These are reused regression sources, not an untouched final test. Require the same ≤0.5% aggregate/per-source perceptual regression and ≤0.002 per-source fine-correlation loss. No post-result tuning sweep. Export every consecutive frame through the native pipeline and score the fixed packet positions. Keep shipping as the concurrent control. The score and stage timing remain separate from browser cadence and physical display claims.

The wider native export completes all 960 pairs. All 960 shipping NV12 payloads and parsed headers exactly reproduce the prior native baseline (JSON header key ordering differs, as expected). Across 32 paired conditions, each measuring 292 frames after eight warmups, preprocessing/reconstruction/detail averages 16.87 ms shipping versus 10.17 ms candidate, a 39.7% reduction. The host was busy and conditions execute sequentially; log percentiles are per-condition, not pooled. Packet packing, capture/decode and rendering remain excluded. Full native logs and source/binary hashes are retained. Wider spatial quality is pending; this timing alone does not authorize integration.
...
## Wider regression result: reject advancement

The RTX 4080 completes all 1,920 scores (960 pairs) with exit zero. Recomputed shipping metrics reproduce the prior CUDA native baseline exactly. Average LPIPS improves 4.82% and DISTS 2.18%, but the unchanged per-source guards fail. Fine-correlation changes are −0.00529 OldTownCross, −0.00269 PedestrianArea, −0.03148 RushHour, −0.01821 Sunflower and −0.01547 Tractor. RushHour LPIPS regresses 2.30% and Sunflower 0.92%. DucksTakeOff, ParkJoy and InToTree pass the per-source screen.

The broader evidence overrides the earlier three-source pass. The prototype does not advance to browser execution or bundled-model promotion. The isolated test app and tested opt-in gain support remain available; no installed shipping resources changed. No further sharpening/coefficient sweep follows. The speed measurements remain valid for their declared stage boundaries, but do not establish a shipping quality/performance win.

Next investigate preserving the existing order of 4× reconstruction, native color conversion, detail processing and final presentation while reducing transfer/materialization costs. Moving downsampling ahead of those stages is not quality-equivalent on the wider set. That observed distinction is a better basis for the next performance experiment than more tuning on these sources.
