...
### r25: Xiph masters re-encoded (bank v4c; receipts `paired-ladder/v4c_*`)

Bank v4 rebuilt with the 12 lossless Xiph excerpts passed through libx264 CRF 17 first, everything else identical. `v4c_w0075_2k` dev +16.84 / +18.82, bank +18.34 / +10.47; `v4c_w005_2k` dev +15.27 / +17.40, bank +17.71 / +9.96. Indistinguishable from bank v4 (r17: +17.11 / +19.08, +18.46 / +10.61). The pristine-master hypothesis is closed with a receipt: the reference encoding was never the variable.

### Temporal feedback probe (receipt `paired-ladder/native-v4-2k-fb075-perceptual-gate.json`)

`v4_2k` through the app with `taaFeedback` 0.75 instead of 0.5: +11.24 / +15.55 (0.5: +11.42 / +15.89), Sunflower −6.1 (−6.4), RushHour +7.2 (+2.8), Tractor +0.3 (+3.7). More history trades Tractor for RushHour and leaves Sunflower where it was. Not the lever.

The absolute view, same checkpoint, torch → app per source (LPIPS): the app improves both arms on the structured sources (old_town_cross candidate −17%, SPAN −11%; in_to_tree −7% / −3%) and worsens both on Sunflower (candidate +15%, SPAN +10%) and Tractor (+9% / +0%). DISTS worsens for both arms on every source, Sunflower most (+39% / +55%). So the stages are not merely "helping SPAN more": on grain they make the picture worse in absolute terms, for every model, and the candidate more so because it carries more fine energy into them. Attribution run in progress: `v4_2k` with every post-stage off (`v4-2k-native-config-off.json`), to separate the stages from the decode and packing path.
...
Partial keep 0.4 (receipt `paired-ladder/native-big2k-grainkeep04-*`): +11.56 / +15.36 against SPAN, under shipping's +12.26 / +16.10; absolute mean −0.2% LPIPS / −1.7% DISTS with pedestrian_area, rush_hour and sunflower each 0.7 to 1.2% worse on LPIPS. It fails the no-source-down rule at every strength, so the bypass is removed from the tree rather than shipped as a knob (revert of 16cee33). Closed: on compressed video, grain and codec noise share the band the accumulator averages, and the accumulator is right to average it.
