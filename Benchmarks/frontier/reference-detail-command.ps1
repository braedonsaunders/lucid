...
$expected=@{
    'Tools\experiments\train_presented_detail.py'='d909a642420d6de7f93b6e3749263566b086a7f76aa5b063e9fb723bd1b83dcd';
...
$labels=if($Mode -eq 'smoke'){@('coupled-smoke','reference-smoke')}else{@('reference')}
...
function Check-Smoke {
    ...
    $candidate=Get-Content 'reference-smoke/experiment.json' -Raw | ConvertFrom-Json
    ...
    foreach($label in @('coupled-smoke','reference-smoke')){
...
try {
    ...
    foreach($label in $labels){
        $detailTarget=if($label -eq 'coupled-smoke'){'mixture'}else{'reference'}
        $steps=if($Mode -eq 'smoke'){'2'}else{'8000'}
        $arguments=$shared+@('--architecture','coupled','--detail-target',$detailTarget,'--steps',$steps,'--out',$label)
