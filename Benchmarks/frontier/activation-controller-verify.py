...
def main():
    ...
    for label in ('static', 'dynamic'):
        ...
        if checkpoint['controller_config']['dynamic'] != (label == 'dynamic'):
        ...
        smoke = json.loads((Path(__file__).parent/f'activation-controller-{label}-smoke-experiment.json').read_text())
        for field in FIELDS:
