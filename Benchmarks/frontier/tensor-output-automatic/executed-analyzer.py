...
def analyze(report_path, minimum_seconds=600, minimum_fps=57, maximum_p95_ms=50, require_source_timestamps=False):
    ...
    for run in report['runs']:
        ...
        source_cadence = None
        if require_source_timestamps or any('sourceTimestamp' in row for row in records):
            if any(not finite(row.get('sourceTimestamp')) or row['sourceTimestamp'] < 0 for row in records):
                raise ValueError('complete finite nonnegative source timestamps required')
            # Count changes across consecutive draws, including the preceding
            # draw at the measurement boundary. A looping clip can revisit PTS.
            previous_pts = None
            changed, repeated, resets = 0, 0, 0
            for row in records:
                pts = row['sourceTimestamp']
                if start <= row['at'] < end:
                    if pts == previous_pts:
                        repeated += 1
                    else:
                        ...
                        if previous_pts is not None and pts < previous_pts:
                            resets += 1
                previous_pts = pts
            source_cadence = {'changed_timestamp_draws': changed, 'repeated_timestamp_draws': repeated,
                              'changed_timestamp_fps': changed / duration, 'backward_timestamp_changes': resets,
                              'scope': 'consecutive source PTS changes; loops/seek resets counted, not pixel uniqueness or physical scanout'}
            if require_source_timestamps:
                gates['source_cadence'] = changed / duration >= minimum_fps
        bins = []
    ...
    return {'report_sha256': hashlib.sha256(report_path.read_bytes()).hexdigest(),
            ...
            'limits': {'minimum_seconds': minimum_seconds, 'minimum_fps': minimum_fps, 'maximum_p95_ms': maximum_p95_ms,
                       'require_source_timestamps': require_source_timestamps},
...
if __name__ == '__main__':
    ...
    parser.add_argument('--require-source-timestamps', action='store_true')
    parser.add_argument('--minimum-seconds', type=float, default=600)
    parser.add_argument('--minimum-fps', type=float, default=57)
    parser.add_argument('--maximum-p95-ms', type=float, default=50)
    args = parser.parse_args()
    if any(not finite(value) or value <= 0 for value in
           (args.minimum_seconds, args.minimum_fps, args.maximum_p95_ms)):
        parser.error('duration, cadence and latency limits must be finite and positive')
    args.out.write_text(json.dumps(analyze(args.report, minimum_seconds=args.minimum_seconds,
        minimum_fps=args.minimum_fps, maximum_p95_ms=args.maximum_p95_ms,
        require_source_timestamps=args.require_source_timestamps), indent=2) + '\n')
