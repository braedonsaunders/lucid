...
Set-Location 'C:\lucid\past-information-20260905-r2'
...
try {
 ...
 $p=Start-Process C:\lucid\.venv\Scripts\python.exe -ArgumentList @('-m','unittest','discover','-s','Tools/experiments','-p','test_past_information.py','-v') -RedirectStandardOutput 'tests.log' -RedirectStandardError 'tests.err' -Wait -PassThru
 if($p.ExitCode -ne 0){throw 'Probe math/alignment tests failed'}
 $arguments=@('-u','Tools/experiments/probe_past_information.py','--device','cuda','--bank','C:\lucid\presented-diversity-20260905\data\bank','--shipping-cache','C:\lucid\presented-diversity-20260905\shipping-cache','--out','result')
