#!/usr/bin/env python3
"""Reproduce the frozen activation-controller experiment's development gates, never promotion."""
import argparse
import hashlib
import json
import math
...
def score(report, control, mode, checkpoints):
    ...
    if {r['variant'] for r in report['rows']} != {'static', 'dynamic', 'shipping', 'lanczos'}:
    ...
    results = {label: compare_rows(report['rows'], expected, GATE,
        candidate=label, other_labels=('lanczos',)) for label in ('static', 'dynamic')}
    ...
    return {
        ...
        'dynamic_minus_static': {
            metric: results['dynamic']['source_balanced']['dynamic'][metric]
                - results['static']['source_balanced']['static'][metric]
...
def main():
    ...
    report = directory / f'activation-controller-{args.mode}.json'
    control = directory / f'anchored-lowpass-{args.mode}.json'
    paths = {label: args.checkpoints / label / 'step008000.pth' for label in ('static', 'dynamic')}
    ...
    (directory/f'activation-controller-{args.mode}-gate.json').write_text(json.dumps(result, indent=2, allow_nan=False)+'\n')
