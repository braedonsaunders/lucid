#!/usr/bin/env python3
"""Reproduce the frozen subspace experiment's development gates, never promotion."""
import argparse
import hashlib
import json
import math
...
def score(report, control, mode, checkpoints):
    ...
    if {r['variant'] for r in report['rows']} != {'full', 'protected', 'shipping', 'lanczos'}:
    ...
    results = {label: compare_rows(report['rows'], expected, GATE,
        candidate=label, other_labels=('lanczos',)) for label in ('full', 'protected')}
    ...
    return {
        ...
        'results': results,
        'protected_minus_full': {
            metric: results['protected']['source_balanced']['protected'][metric]
                - results['full']['source_balanced']['full'][metric]
...
def main():
    ...
    report = directory / f'subspace-{args.mode}.json'
    control = directory / f'anchored-lowpass-{args.mode}.json'
    paths = {label: args.checkpoints / label / 'step008000.pth' for label in ('full', 'protected')}
    ...
    (directory/f'subspace-{args.mode}-gate.json').write_text(json.dumps(result, indent=2, allow_nan=False)+'\n')
