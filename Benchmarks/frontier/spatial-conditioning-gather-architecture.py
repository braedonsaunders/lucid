...
class SpatialActivationControl(ActivationControl):
    def __init__(self, anchor, selection, rms, dynamic=True, window=9):
        ...
        self.window = window
        # The masks are fixed one-hot selectors, not learned projections.
        # Slot zero is the identity channel; selected channels use slots 1..4.
        indices = (self.masks * torch.arange(1,5)[None,:,None]).sum(1).long()
        self.register_buffer('coefficient_indices', indices, persistent=False)
    ...
    def forward(self, x):
        ...
        for i, block in enumerate((core.block_1, core.block_2, core.block_3,
            ...
            zero = torch.zeros_like(coefficients[:, i, :1])
            gains = torch.cat((zero, coefficients[:, i, :4]), 1).index_select(1, self.coefficient_indices[i])
            offsets = torch.cat((zero, coefficients[:, i, 4:]), 1).index_select(1, self.coefficient_indices[i])
            scale = 1 + .5 * gains
            shift = .2 * offsets * self.shift_units[i][None,:,None,None]
            current = current * scale.to(current.dtype) + shift.to(current.dtype)
            if i == 0:
                first = current
