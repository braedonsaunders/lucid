...
- **Corrected full-4× tensor output matches all 960 native pairs exactly:** shared Metal output storage preserves full reconstruction, color conversion, detail and sender output. The eight-source regression has identical final NV12 payloads and parsed headers in every pair. The native enhancement-stage mean is 26.3% lower on this shared host; browser comparison is pending. [Protocol and evidence](tensor-output-protocol.md). Controlled synthetic audits identified the required binary16 rounding and non-IOSurface backing; the earlier half-up/IOSurface implementation remains rejected. This is an implementation speedup with unchanged weights, not improved learned quality or release admission.
