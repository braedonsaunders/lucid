# Lucid development evidence — September 5, 2026
...
- **Quality replacements still fail promotion.** The [native eight-source result](native-quality-holdout.md), broader-data models, reference-detail supervision, bounded adversarial training, constrained correction branches and subspace adapters did not establish a joint winner. The latest [matched activation-controller experiment](activation-controller-protocol.md) completes both 8,000-step RTX 4080 arms: dynamic conditioning improves full-frame LPIPS/DISTS 5.70%/6.24%, but both variants fail source detail guards. A fresh matched diagnostic attributes the additional loss to training, while the folded initialization itself passes those detail limits. The completed [local-fidelity experiment](local-fidelity-protocol.md) preserves all per-source detail guards on both sets but improves perception by only about 1%, below the 3% gate; neither matched arm is promoted. The completed [coarse controller](spatial-conditioning-protocol.md) also misses the 3% perceptual gates. The [learned past-information diagnostic](learned-past-information-protocol.md) improves detail correlation but misses its fixed 1% MSE gate versus the current-only control. Shipping weights remain unchanged.
- **Presentation prototype rejected by wider regression:** unchanged shipping weights with in-graph RGB8 bicubic downsampling reduce a native graph/packet stage from 19.38 to 9.95 ms (48.6%) in a short paired test. Correcting scale-dependent sharpening gain passes the separate 48-pair native delivered screen: LPIPS +4.38%, DISTS +2.88%, detail correlation improves on every source. [Full evidence and limits](quantized-presentation-protocol.md). The subsequent 960-pair, eight-source native regression improves averages but fails five detail guards and two LPIPS guards; it does not advance to browser testing or production. No end-to-end speedup or release admission is established.
- **Corrected full-4× tensor output preserves native pixels and improves measured delivery:** all 960 regression packet pairs and 16 additional 480p fixture pairs are identical to shipping. The shared-host native stage mean is 26.3% lower. A headed installed-companion run sustains 59.839 distinct-source-PTS frames/s with 30.800ms capture-to-canvas p95 for 618.809 seconds; the shorter ABBA also favors the candidate. [Protocol and evidence](tensor-output-protocol.md). The [automatic integration](tensor-output-automatic-protocol.md) now checks all six actual bundled models before selection on M4 Pro/macOS 26.5.1 and retains image fallback. Its new 622-second 480p run delivers 29.992 distinct-PTS fps at 33.300 ms p95; clean short ABBA passes at 360p60 and 480p30. All 46 native tests pass. Weights remain unchanged. This is an implementation speedup, not a learned-quality or novelty claim.
...
- **A commercial reference was measured on the identical 48-pair screen** and used to calibrate the perceptual gate; its numbers are not published here because the vendor's SDK licence restricts disclosing benchmark results.
- **Latest external comparisons:** [PiperSR](2026-native-deployment-followup.md) fails the matched native quality gate and is slower than shipping on this M4 Pro. The author's [2026 PLKSR-Rep release](plksr-rep-protocol.md) improves LPIPS 15.36% but worsens DISTS 7.29% and fails every source's fine-detail guard. Its correct FP32 native graph takes about 297 ms at 360p; tested FP16 exports fail RGB accuracy limits. Neither release qualifies as a replacement here.
- **Other released baseline:** [July 2026 NanoVSR](nanovsr-baseline.md), pinned released weights with strictly loaded parameters. Its past-only execution improves development LPIPS 5.55% and DISTS 3.57% but fails all three source fine-detail guards; it is not promoted. Official bidirectional execution also fails.
...
| Native and browser performance | On the target M4 Pro, measured installed-extension capture-to-draw p95 ≤50 ms and ≥95% of source cadence over 10 minutes for 360p/480p 30 fps; 360p60 must deliver ≥57 fps. Include queue depth, drops, warmup and steady memory. | Direct surface route passes 360p60 installed-Chrome draw gate: 59.47 fps / 40.60 ms p95 over 621 s. A second 620 s source-PTS gate passes at 58.38 fps / 40.90 ms. 480p30 also passes over 617 s at 29.99 fresh-timestamp fps / 39.30 ms p95. Higher resolutions, Safari and full browser memory checks remain. |
| Resolution coverage | Efficient, useful 720p→1440p and 1080p→2160p routes at ≥95% of 30 fps cadence, with quality gains over native interpolation. Measure the actual intended routes before admitting them. | Current shipping guard covers 144–480p SDR. The [720p folded-head coverage screen](coverage-720-protocol.md) fails: LPIPS improves 9.38% versus Lanczos but DISTS worsens 9.04%, with a DucksTakeOff fine-detail regression. |
