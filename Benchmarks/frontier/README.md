...
- **Current priority: quality/performance; flicker work deferred at the user's direction.** [Persistent Core ML state](causal-native-runtime.md) reduces native Swift experimental 1080p→4K prediction time 35.3%; full-resolution NV12 packet creation is also measured. The matched width-expansion trial gives only 0.078% additional LPIPS improvement, so the faster 32-channel route remains preferred. [Official August-2026 PixRestore-S](pixrestore-baseline.md), evaluated in 512-pixel tiles, improves development LPIPS 20.0%, DISTS 11.3% and aggregate fine-detail correlation 3.2% over Lanczos. A reference-checked distillation pilot is being prepared; neither teacher nor student is promoted.
