...
- **Current priority: quality/performance; flicker work deferred at the user's direction.** The [direct extension-surface capture route](surface-capture-transport.md) removes a frame serialization hop. Installed Chrome with shipping weights delivered 59.47 draw acknowledgments/s over 621 seconds at 40.60 ms p95. A same-session route comparison improved pooled draw cadence 29.6%. Source-timestamp validation is separate; these draw counts do not establish unique source-frame cadence or Safari performance.
