...
- **Promising presentation prototype:** unchanged shipping weights with in-graph RGB8 bicubic downsampling reduce a native graph/packet stage from 19.38 to 9.95 ms (48.6%) in a short paired test. Correcting scale-dependent sharpening gain passes the separate 48-pair native delivered screen: LPIPS +4.38%, DISTS +2.88%, detail correlation improves on every source. [Full evidence and limits](quantized-presentation-protocol.md). This is not a measured end-to-end speedup or release admission.
