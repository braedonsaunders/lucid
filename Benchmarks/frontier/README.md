...
- **Full-4× tensor-output implementation rejected by wider native testing:** replacing Core ML image output with borrowed tensor storage plus Metal RGB8 packing retains the complete reconstruction/color/detail order. The fixed FP32 confirmation reduces the 360p boundary mean 26.5%, with maximum one RGB-level difference. The first actual native 48-pair screen passes the unchanged perceptual/detail noninferiority limits; its short enhancement-stage mean is 11.8% lower. A separate 480p boundary probe is 24.2% faster, but does not establish delivered 480p quality. [Protocol and evidence](tensor-output-protocol.md). This is a measured implementation optimization, not new learned quality or release admission; the 960-pair native regression fails OldTownCross’s fixed DISTS limit despite passing all detail guards. It does not advance to browser testing.
...
- **A real NVIDIA VFX SDK image comparison now runs on the RTX 4080:** the official public wheel supplies SDK 1.2.0. Across the identical-input 48-pair RGB screen, HIGH improves LPIPS/DISTS 23.14%/17.86% over Lucid’s native Core ML model-output comparator; ULTRA improves 25.00%/22.22%. Both lose excess fine correlation on two sources. [Full protocol and lifecycle limitations](nvidia-vfx-protocol.md). A reproduced SDK destructor hang is contained with a fresh process per image; this is not browser-driver RTX VSR or Lucid’s full native pipeline, and no parity claim follows.
