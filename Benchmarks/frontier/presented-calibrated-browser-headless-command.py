...
def main():
    ...
    report={'purpose':'ABBA headless Chrome native playback comparison at 640x360 to 1280x720',
    ...
    try:
        ...
        for index,label in enumerate(report['order']):
            ...
            cli('open','http://127.0.0.1:48113','--browser','chrome','--config',str(args.config.resolve()))
            cli('snapshot')
            setup=js('''async page => {
            ...
            samples=js('''async page => {
              const result=await page.evaluate(async () => {
                const samples=[];
                for(let i=0;i<30;i++) { await new Promise(resolve=>setTimeout(resolve,1000)); samples.push({time:performance.now(),status:window.latestStatus}); }
