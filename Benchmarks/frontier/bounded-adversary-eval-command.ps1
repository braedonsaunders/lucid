...
$result=Get-Content 'bounded-result.json' -Raw | ConvertFrom-Json
...
try {
    ...
    & 'C:\lucid\.venv\Scripts\python.exe' Tools/experiments/interpolate_presented_detail.py --anchor 'C:\lucid\presented-diversity-eval-20260905\area-init.pth' --candidate 'bounded/step008000.pth' --weights 0.8 --out calibrated
    if($LASTEXITCODE -ne 0){throw 'Fixed interpolation failed'}
    $arguments=@('-u','Tools/frontier_eval/score_checkpoint_frames.py','--device','cuda','--frames','C:\lucid\presented-frozen-eval-20260904\frames','--checkpoint','bounded80','calibrated/blend-0.8.pth','--checkpoint','bounded_raw','bounded/step008000.pth','--checkpoint','shipping','C:\lucid\presented-detail-20260904\shipping.pth','--present-4x-at-2x','shipping','--report','evaluation.json')
