...
foreach ($path in @('strong-distilled', 'strong-owner.lock')) {
    if (Test-Path $path) { throw "Preserve existing $path" }
}
New-Item -ItemType File 'strong-owner.lock' -ErrorAction Stop | Out-Null
...
try {
    ...
    Run-Python 'strong-distilled' ($shared + @('--teacher-weight', '10', '--out', 'strong-distilled'))
} finally { Remove-Item 'strong-owner.lock' }
